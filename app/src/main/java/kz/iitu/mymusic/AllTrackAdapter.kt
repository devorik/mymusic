...
class AllTrackAdapter(
    private val track: List<Track?> = listOf()
) : RecyclerView.Adapter<AllTrackAdapter.AllViewHolder>() {

    class AllViewHolder(private val view: View) : RecyclerView.ViewHolder(view) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = AllViewHolder(LayoutInflater.from(parent.context)
        .inflate(R.layout.layout_top_ten_track,parent,false))


    override fun getItemCount() = track.size

    override fun onBindViewHolder(holder: AllTrackAdapter.AllViewHolder, position: Int) {
